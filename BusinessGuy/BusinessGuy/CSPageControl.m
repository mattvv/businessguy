...
- (void)layoutSubviews
{
  ...
  if (dots.count != (NSUInteger)num) {
      [dots makeObjectsPerformSelector:@selector(removeFromSuperview)];
    [dots removeAllObjects];
    
    for (NSInteger ii = 0; ii < num; ii++) {
      UIImageView *dot;
      if (ii == cachedPage) {
        dot = [[UIImageView alloc] initWithImage:self.active];
        if (dot == nil) {
          dot = (UIImageView*)[[UIView alloc] initWithFrame:CGRectZero];
        ...
        dot = [[UIImageView alloc] initWithImage:self.inactive];
        if (dot == nil) {
          dot = (UIImageView*)[[UIView alloc] initWithFrame:CGRectZero];
          dot.backgroundColor = [UIColor clearColor];
          dot.userInteractionEnabled = NO;
